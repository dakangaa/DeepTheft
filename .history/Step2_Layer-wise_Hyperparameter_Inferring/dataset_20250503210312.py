...
class ToTargets(torch.nn.Module):
    ...
    def forward(self, targets):
        # targets：一层的所有超参数
        if self.mode == 'kernel_size' and self.layer_type == "conv2d":
            # {1, 3, 7} --> {0, 1, 2}
            targets = targets[self.label]
            targets = (targets - 1) / 2
            targets = targets - 1 if targets == 3 else targets
        elif self.mode == 'kernel_size' and self.layer_type == "max_pool2d":
            # {2,3} --> {0,1}
            targets = targets[self.label]
            targets = targets - 2
        if self.mode == 'stride':
...
class RaplLoader(object):
    def __init__(self, args, no_val=False, input_size=["224"]):
        ...
        else args.layer_type == "max_pool2d":
        ...
        self.seed = 0
        # 数据预处理
        use_copypad_conv = ["kernel_size", "stride", "out_channels"]
        use_copypad_maxpool = []
        # TEST:
        if args.use_copypad:
            if args.use_norm:
        ...
        # if (self.layer_type == "conv2d" and args.HyperParameter in use_copypad_conv) or (self.layer_type == "max_pool2d" and args.HyperParameter in use_copypad_maxpool):
        #     self.transform = transforms.Compose([
        #         Normalization(), # 归一化
