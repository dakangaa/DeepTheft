...
def test(layer_type, HyperParameters, Origin_domain_nums, test_domain, args):
    ...
    for hp in HyperParameters:
        for v in var2:
            ...
            print("testing...")
            if args.mode == "O":
                file = path + "/" + layer_type + hp + "_" + str(v) + "_" + const_var + "_train_ckpt.pth"
                ...
                file = path + "/" + layer_type + hp + "_" + str(const_var) + "_" + v + "_train_ckpt.pth"
                test_cmd = ["python", "Step2_Layer-wise_Hyperparameter_Inferring/test.py", "--layer_type", layer_type,
    ...
    for hp in hyperParameters:
        for od in origin_domain_nums:
            ...
            if is_regression and hp == "out_channels":
                file = path + '/' + layer_type + hp + "_" + str(od) + "_" + "331" + "_" + "regression" + '_ckpt.pth'
            else:
                file = path + '/' + layer_type + hp + "_" + str(od) + "_" + "331" + "_" + "train" + '_ckpt.pth'
        ...
        for td in test_domain:
            if is_regression and hp == "out_channels":
                file = path + '/' + layer_type + hp + "_" + str(4) + "_" + td + "_" + "regression" + '_ckpt.pth'
            else:
                file = path + '/' + layer_type + hp + "_" + str(4) + "_" + td + "_" + "train" + '_ckpt.pth'
            checkpoint = torch.load(file, map_location=torch.device('cpu'))
            for col in columns:
                if col not in checkpoint.keys():
...
if __name__ == "__main__":
    ...
    if args.mode == "O":
        ...
        with pd.ExcelWriter("results/results.xlsx", if_sheet_exists="replace", mode="a") as writer:
            if args.regression:
                df.to_excel(writer, sheet_name=args.layer_type + "origin_domain_num_regression")
            else:
                df.to_excel(writer, sheet_name=args.layer_type + "origin_domain_num")
    elif args.mode == "T":
        ...
        with pd.ExcelWriter("results/results.xlsx", if_sheet_exists="replace", mode="a") as writer:
            if args.regression:
                df.to_excel(writer, sheet_name=args.layer_type + "test_domain_regression")
            else:
                df.to_excel(writer, sheet_name=args.layer_type + "test_domain")
    else:
        raise ValueError
