...
def print_hdf5_structure(item, indent=0):
    ...
    if isinstance(item, h5py.File):
        print(" " * indent + f"{type(item)} : {item.name}")
        for key in item.keys():
            print_hdf5_structure(item[key], indent+4)
...
# 打开 HDF5 文件
file_path = './dataset/conv2d.h5'
with h5py.File(file_path, 'r') as f:
    print(f"文件结构: {file_path}")
    print_hdf5_structure(f["trace"])